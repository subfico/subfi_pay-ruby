...
> resend_webhook_request(id, x_api_version, x_account_id)
...
begin
  # Resend webhook request by ID
  api_instance.resend_webhook_request(id, x_api_version, x_account_id)
rescue SubfiPay::ApiError => e
...
> <Array(nil, Integer, Hash)> resend_webhook_request_with_http_info(id, x_api_version, x_account_id)

```ruby
begin
  # Resend webhook request by ID
  data, status_code, headers = api_instance.resend_webhook_request_with_http_info(id, x_api_version, x_account_id)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => nil
rescue SubfiPay::ApiError => e
